...
#########  Controlling effects from different vintages of adpotion  ##########
##############################################################################
...
#
# OUTPUT FILES 
# - DiD_results/vintages_exercise/spike2_autn_rob_ai_ln_dip_CS_att_results.RData
...
#
#   and aggregating the AAA to extract the contribution of specific vintages
...
folder_exercise <- 'vintages_exercises'
...
estimates_dt <- estimation_calendar(redux_sample,
...
formula_string <- '~ is_exporter + first_size_class + initial_gr_dip + ln_prod + ln_sales + sh_titolo_low + sh_White_collar + sh_full_time' 
estimates_dt_oa <- estimation_calendar(redux_sample[is_adopter == 1],
                              identification_spike,
...
wgt_estimates_dt <- estimation_calendar(redux_sample,
                               identification_spike,
                               vr, tp, rec,
                               control_type = 'nevertreated', 
                               anticipation_num,
...
wgt_estimates_dt_oa <- estimation_calendar(redux_sample[is_adopter == 1],
                                  identification_spike,
                                  vr, tp = 'Total economy - Adopters only', rec,
                                  control_type = 'notyettreated', 
                                  anticipation_num,
